...
void ProcessLabeling()
{
	...
	const double threshold2 = 20.0f/640.0f; // 50 pixels
					...
					if (depth_image.at<unsigned short>(i, j) < threshold1){
...

						{
							...
							if ( (line_seg_point_distance(gKinectSkelton.positions[NUI_SKELETON_POSITION_SHOULDER_RIGHT], 
								...
								memcpy(&culustered_image.data[(j + i*IMAGE_WIDTH) * 3], colors[0], 3);
								/*
								line(culustered_image,
								...
								*/
								...
								memcpy(&culustered_image.data[(j + i*IMAGE_WIDTH) * 3], colors[1], 3);
							} // left foot
								...
								memcpy(&culustered_image.data[(j + i*IMAGE_WIDTH) * 3], colors[2], 3);
							} // right foot
								...
								memcpy(&culustered_image.data[(j + i*IMAGE_WIDTH) * 3], colors[3], 3);
							} // head
								...
								memcpy(&culustered_image.data[(j + i*IMAGE_WIDTH) * 3], colors[4], 3);
							} // body
							else if (line_seg_point_distance(gKinectSkelton.positions[NUI_SKELETON_POSITION_SPINE],
								gKinectSkelton.positions[NUI_SKELETON_POSITION_SHOULDER_CENTER], current) < threshold2 * 3){
								memcpy(&culustered_image.data[(j + i*IMAGE_WIDTH) * 3], colors[5], 3);
							}
						}
					}
				}
