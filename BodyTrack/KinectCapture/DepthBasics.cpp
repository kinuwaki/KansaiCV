        ...
        case WM_COMMAND:
            ...
            if (IDC_CHECK_NEARMODE == LOWORD(wParam) && BN_CLICKED == HIWORD(wParam))
            {
				...
				if (NULL != m_pNuiSensor)
				{
					...
					m_pNuiSensor->NuiSkeletonTrackingEnable(m_pDepthStreamHandle, 0 ? NUI_SKELETON_TRACKING_FLAG_ENABLE_SEATED_SUPPORT : 0);
				}
			}
            break;
    }
...
HRESULT CDepthBasics::CreateFirstConnected()
{
    ...
    if (NULL != m_pNuiSensor)
    {
        ...
        hr = m_pNuiSensor->NuiInitialize(NUI_INITIALIZE_FLAG_USES_DEPTH | NUI_INITIALIZE_FLAG_USES_SKELETON); 
        if (SUCCEEDED(hr))
        {
		...
		hr = m_pNuiSensor->NuiSkeletonTrackingEnable(m_hNextDepthFrameEvent, 0); 
...
void CDepthBasics::ProcessDepth()
{
	...
	// kinuwaki : perform frame dump
    if ( (LockedRect.Pitch != 0) && (idxFrame % 100 == 98) )
    {
		char filename[100];
		sprintf(filename, "frame%.3d.txt", idxFrame / 100);
		fp = fopen(filename, "w");
